"""
Migration script to add diagnoses_data column to consultation_templates table (MySQL)
"""
import pymysql
import os
import sys
...
def migrate():
    ...
    try:
        ...
        if not table_exists:
            print("✗ Error: Table 'consultation_templates' does not exist.")
            print("Please run 'migrate_create_consultation_templates_table_mysql.py' first.")
            sys.exit(1)
        ...
        try:
            cursor.execute("""
                ...
                WHERE TABLE_SCHEMA = %s 
                AND TABLE_NAME = 'consultation_templates'
                AND COLUMN_NAME = 'diagnoses_data'
            """, (db_name,))
            result = cursor.fetchone()
        ...
        if column_exists:
            print("✓ Column 'diagnoses_data' already exists in 'consultation_templates' table")
        else:
            # Add diagnoses_data column
            print("Adding 'diagnoses_data' column to 'consultation_templates' table...")
            cursor.execute("""
                ALTER TABLE consultation_templates 
                ADD COLUMN diagnoses_data LONGTEXT NULL
                AFTER investigations_data
            """)
            print("✓ Added 'diagnoses_data' column to 'consultation_templates' table")
