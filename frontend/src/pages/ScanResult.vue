<template>
  <q-page class="hms-page">
    <HmsPageHeader title="Scan result" subtitle="Enter or review imaging findings.">
      <template #actions>
        <HmsButton variant="secondary" size="sm" @click="$router.push('/scan')">
          Back
        </HmsButton>
      </template>
    </HmsPageHeader>
    ...
    <section v-if="investigation" class="result-panel">
      ...
      <div class="result-meta-grid">
        ...
        <div class="result-meta-item">
          ...
          <div class="result-meta-value">{{ scanResult?.entered_by_name || 'N/A' }}</div>
        </div>
        <div class="result-meta-item">
          <div class="result-meta-label">Updated by</div>
          <div class="result-meta-value">{{ scanResult?.updated_by_name || 'N/A' }}</div>
        </div>
      </div>
    ...
    <section class="result-panel result-form-panel">
      <div class="result-panel-head">
        ...
        <p class="result-panel-sub">Enter scan findings and attach supporting files.</p>
      </div>
      <div
      ...
      >
        ...
        <div>
          This investigation is completed. Only Admin and Scan Head can edit completed investigations. Please contact Scan Head to revert the status if changes are needed.
        </div>
      </div>
      <q-form @submit="saveScanResult" class="q-gutter-md">
          <q-input
            ...
            rows="10"
            label="Results Text"
            hint="Enter scan results/findings"
          ...
          <div class="result-form-actions">
            <HmsButton variant="secondary" @click="$router.push('/scan')">
              Cancel
            </HmsButton>
            <HmsButton
              variant="primary"
...
const canEditResult = computed(() => {
  ...
  if (investigation.value.status === 'completed') {
    return authStore.userRole === 'Admin' || authStore.userRole === 'Scan Head';
  }
  // For non-completed investigations, all Scan staff can edit
  return true;
});
...
const loadInvestigation = async () => {
  ...
  try {
    ...
    try {
      const resultResponse = await consultationAPI.getScanResult(investigation.value.id);
      ...
      if (existingResult && existingResult.investigation_id === investigation.value.id) {
        scanResult.value = existingResult;
      ...
      console.log('No scan result found for investigation:', investigation.value?.id || route.params.investigationId, 'Source:', investigation.value?.source || 'unknown', error);
      scanResult.value = null;
      editingResult.value = false;
      resultForm.value = {
        investigation_id: investigation.value?.id || parseInt(route.params.investigationId),
