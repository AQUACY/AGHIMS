<template>
  <q-page class="hms-page">
    <HmsPageHeader title="Lab result" subtitle="Enter or review laboratory findings.">
      <template #actions>
        <HmsButton variant="secondary" size="sm" @click="$router.push('/lab')">
          Back
        </HmsButton>
      </template>
    </HmsPageHeader>
    ...
    <section v-if="investigation" class="result-panel">
      ...
      <div class="result-meta-grid">
        ...
        <div v-if="investigation.confirmed_by_name" class="result-meta-item">
          <div class="result-meta-label">Confirmed by</div>
          <div class="result-meta-value">{{ investigation.confirmed_by_name }}</div>
        </div>
        <div v-if="investigation.completed_by_name" class="result-meta-item">
          <div class="result-meta-label">Completed by</div>
          <div class="result-meta-value">{{ investigation.completed_by_name }}</div>
        </div>
        <div v-if="labResult?.entered_by_name" class="result-meta-item">
          <div class="result-meta-label">Entered by</div>
          <div class="result-meta-value">{{ labResult.entered_by_name }}</div>
        </div>
        <div v-if="labResult?.updated_by_name" class="result-meta-item">
          <div class="result-meta-label">Updated by</div>
          <div class="result-meta-value">{{ labResult.updated_by_name }}</div>
        </div>
      </div>
    ...
    <section class="result-panel result-form-panel">
      <div class="result-panel-head">
        ...
        <p class="result-panel-sub">Enter laboratory findings for this investigation.</p>
      </div>
      <div
      ...
      <q-form @submit="saveLabResult" class="q-gutter-md">
          ...
          <div v-if="usingTemplate && template">
            ...
            <div
              ...
              class="result-warn-banner result-warn-banner--critical"
              role="alert"
            >
          ...
          <div class="result-form-actions">
            <HmsButton variant="secondary" @click="$router.push('/lab')">
              Cancel
            </HmsButton>
            <HmsButton
              variant="primary"
              ...
              :disabled="!canEditResult || !canEnterResults"
            >
              Save
            </HmsButton>
          </div>
...
.result-warn-banner--critical {
  background: var(--hms-critical-muted, rgba(239, 68, 68, 0.1));
