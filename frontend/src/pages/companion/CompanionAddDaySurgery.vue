    ...
    <q-card class="glass-card q-mb-lg" flat>
      <q-card-section>
        ...
        <q-input
          ...
          filled
          dense
          placeholder="Type day surgery name or code..."
          class="q-mb-sm"
          clearable
        ...
        <div class="row q-col-gutter-md q-mb-md">
          <div class="col-12 col-md-6">
            <q-select
              ...
              filled
              dense
              label="Add day surgery to card list"
              use-input
              input-debounce="200"
            ...
            >
              <template v-slot:no-option>
                <q-item>
                  <q-item-section class="text-grey">Type to search DAY SURGERY</q-item-section>
                </q-item>
              </template>
            </q-select>
          </div>
...
const SERVICE_TYPE = 'DAY SURGERY';
...
async function loadAddedItems() {
  try {
    const res = await companionVisitsAPI.getItems(visitId.value, 'day_surgery');
...
async function loadActiveDaySurgeries() {
  loadingActive.value = true;
  try {
    const res = await companionVisitsAPI.getActiveDaySurgeries();
...
async function onAddToCardList(gDrgCode) {
  if (!gDrgCode) return;
  try {
    await companionVisitsAPI.addActiveDaySurgery({ g_drg_code: gDrgCode });
    $q.notify({ type: 'positive', message: 'Added to card list', position: 'top' });
    addActiveProc.value = null;
    await loadActiveDaySurgeries();
  } catch (e) {
    $q.notify({
      type: 'negative',
...
async function removeActive(gDrgCode) {
  try {
    await companionVisitsAPI.removeActiveDaySurgery(gDrgCode);
