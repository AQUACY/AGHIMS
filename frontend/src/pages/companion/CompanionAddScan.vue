<template>
  <q-page class="q-pa-md">
    <div class="row items-center q-mb-md">
      ...
      <div class="text-h5 text-weight-bold glass-text q-ml-sm">Add scan</div>
    </div>

    <q-card v-if="visitClosed" class="glass-card q-mb-md" flat>
      <q-card-section>
        <q-banner class="bg-warning/20 text-warning rounded-borders">
          This visit is closed. You cannot add or remove scans.
        </q-banner>
      </q-card-section>
    </q-card>
    ...
    <!-- Search and add (any scan) -->
    <q-card class="glass-card q-mb-lg" flat>
      <q-card-section>
        <div class="text-subtitle1 text-weight-medium glass-text q-mb-md">Search and add scan</div>
        <div class="text-caption glass-text-muted q-mb-sm">Type to search, then select to add to this visit.</div>
        <q-input
          v-model="searchText"
          filled
          dense
          placeholder="Type scan name or code..."
          class="q-mb-sm"
          clearable
        />
    ...
    <!-- Regularly requested (card list – Scan Head activates these) -->
    ...
    <div v-else-if="procedures.length === 0" class="text-body2 glass-text-muted">
      No scans in the price list for service type "ULTRASOUND".
    </div>
    <div v-else-if="activeProcedures.length === 0" class="text-body2 glass-text-muted">
      No scans are on the card list yet. Scan Head can activate regularly requested ones below. Use "Search and add" above to add any scan to the visit.
    </div>
    <div v-else class="row q-col-gutter-lg q-ma-md">
      ...
      >
        <q-card-section class="text-center q-pa-md">
          <q-icon name="document_scanner" size="32px" class="text-primary" />
    ...
    <!-- Scan Head: manage card list -->
    <q-card v-if="canManageActive" class="glass-card q-mt-xl" flat>
      <q-card-section>
        <div class="text-h6 glass-text q-mb-md">Manage card list (Scan Head)</div>
        <div class="text-caption glass-text-muted q-mb-md">Scans on the card list appear above. Add or remove them here.</div>
          ...
          <div class="col-12 col-md-6">
            ...
            >
              <template v-slot:no-option>
                <q-item>
                  <q-item-section class="text-grey">Type to search ULTRASOUND</q-item-section>
                </q-item>
              </template>
            </q-select>
          </div>
        ...
        <div v-else class="text-caption text-grey-7">No scans on the card list yet.</div>
      </q-card-section>
    </q-card>
  </q-page>
</template>
...
// One card per active scan (price list can have multiple procedure rows per g_drg_code, e.g. same scan “with other services”)
...
async function loadAddedItems() {
  try {
    const res = await companionVisitsAPI.getItems(visitId.value, 'scan');
...
async function loadActiveScans() {
  ...
  try {
    const res = await companionVisitsAPI.getActiveScans();
...
async function onAddToCardList(gDrgCode) {
  ...
  if (!code) return;
  try {
    await companionVisitsAPI.addActiveScan({ g_drg_code: String(code) });
...
async function removeActive(gDrgCode) {
  try {
    await companionVisitsAPI.removeActiveScan(gDrgCode);
