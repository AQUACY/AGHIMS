  ...
  <q-page class="hms-page aiv-page">
    ...
    >
      ...
      <p class="aiv-hero__lede">
        ...
        nothing changes until a human approves. For local Llama/Ollama reviews, open
        Local AI Assist instead.
      </p>
      <div v-if="moduleActive" class="aiv-hero__status">
        <span class="aiv-chip" data-tone="ok">Human approval required</span>
        <span class="aiv-chip" data-tone="muted">Rules engine · Phase / Coding / Thorough</span>
    ...
    <template v-else-if="moduleActive">
      ...
      >
        ...
        <p v-if="jobRunning" class="aiv-muted q-mt-sm">
          Findings appear in the report below as each claim finishes. Phase scans use the fast rules engine.
          Use Local AI Assist for Ollama recommendations on selected claims.
        </p>
              ...
              <template v-else>
                <div class="aiv-detail__head">
                  ...
                  <div class="aiv-detail__actions">
                    ...
                    <button
                      v-if="isReviewOnlyGroup(activeGroup.rule_code)"
                      type="button"
                      class="aiv-primary aiv-primary--sm"
                      :disabled="!selectedInGroup(activeGroup).length || correcting"
                      @click="bulkReopenEdited(selectedInGroup(activeGroup))"
                    >
                      Reopen selected as AI vetted
                    </button>
                  </div>
                </div>
                ...
                <div class="aiv-cards">
                  ...
                  >
                    ...
                    <div class="aiv-card__foot">
                      ...
                      <div v-else-if="isValueFixFinding(row)" class="aiv-card__drg">
                        <label class="aiv-card__drg-label">
                          Correct {{ row.suggested_action?.field || 'value' }}
                        </label>
                        <input
                          class="aiv-nav__input"
                          :value="chosenValue[row.id] || ''"
                          :placeholder="row.member_no || 'Enter correct value'"
                          @input="setChosenValue(row.id, $event.target.value)"
                        />
                      </div>
                      <div class="aiv-card__btns">
                        ...
                        <button
                          v-else-if="isValueFixFinding(row)"
                          type="button"
                          class="aiv-primary aiv-primary--sm"
                          :disabled="correcting || !(chosenValue[row.id] || '').trim()"
                          @click="acceptValueFinding(row)"
                        >
                          Apply value
                        </button>
                        <button
                        ...
                        >
                          {{ findingNeedsReopen(row) ? 'Reopen as AI vetted' : 'Mark edited' }}
                        </button>
                        <button
                          v-else
                          type="button"
      ...
      >
        <div class="aiv-panel__head">
          <div>
            ...
            <p class="aiv-muted q-mt-xs">
              Write what you want in plain English. Local AI drafts the rule — you review and approve before it runs.
            </p>
          </div>
        ...
        <div class="aiv-plain-rule">
          <label class="aiv-field aiv-field--wide">
            <span>Describe the rule in plain English</span>
            <textarea
              v-model="rulePlainEnglish"
              class="aiv-textarea"
              rows="3"
              placeholder="Example: If Member No starts with a hyphen, strip it. Skip Ghana Cards."
            />
          </label>
          <div class="aiv-cta-row">
            <button
              type="button"
              class="aiv-primary"
              :disabled="draftingRule || !(rulePlainEnglish || '').trim()"
              @click="draftRuleFromEnglish"
            >
              {{ draftingRule ? 'Drafting with local AI…' : 'Draft rule with AI' }}
            </button>
          </div>
          <p v-if="ruleDraftNote" class="aiv-muted q-mt-sm">{{ ruleDraftNote }}</p>
        </div>
...
const draftingRule = ref(false);
const rulePlainEnglish = ref('');
const ruleDraftNote = ref('');
...
function ruleLane(ruleCode) {
  ...
  // Custom facility rules default to Phase 1 lane
  return 'phase1';
}
...
const reportLanes = computed(() => {
  const groups = reportGroups.value.filter((g) => !String(g.rule_code || '').startsWith('llm_'));
  const byLane = {
    phase1: [],
    coding: [],
    thorough: [],
  ...
  for (const g of groups) {
    const lane = ruleLane(g.rule_code);
    if (lane === 'ai') continue;
    byLane[lane]?.push({
      ...g,
...
function isValueFixFinding(row) {
  if (!isReviewOnlyFinding(row)) return false;
  const field = row?.suggested_action?.field || '';
  return ['memberNo', 'hin', 'ghanaCard', 'specialtyAttended', 'hospitalRecNo', 'cardSerialNo'].includes(field);
}

function setChosenValue(findingId, value) {
  chosenValue.value = { ...chosenValue.value, [findingId]: value };
}
...
async function loadReport({ quiet = false } = {}) {
  ...
  try {
    ...
    // Don't overwrite an in-flight job with a stale latest_job snapshot.
...
async function pollJob(jobId) {
  ...
  pollTimer = setInterval(async () => {
    try {
      ...
      job.value = res.data;
      ticks += 1;
      // Surface findings as claims finish — don't wait for the whole job.
...
async function confirmReopenIfNeeded(rowOrIds) {
  ...
  return new Promise((resolve) => {
    $q.dialog({
      ...
      message:
        finalized.length === 1
          ? `Claim ${finalized[0].claim_claim_id || finalized[0].source_id} is finalized. It will reopen as ai_vetted (still exportable — not draft).`
          : `${finalized.length} selected claim(s) are finalized. They will reopen as ai_vetted (exportable).`,
      cancel: true,
      persistent: true,
      ok: { label: 'Reopen as AI vetted', color: 'primary' },
    })
      .onOk(() => resolve(true))
...
async function acceptValueFinding(row) {
  const value = (chosenValue.value[row.id] || '').trim();
  if (!value) {
    $q.notify({ type: 'warning', message: 'Enter the corrected value first', position: 'top' });
    return;
  }
  if (!(await confirmReopenIfNeeded(row))) return;
  correcting.value = true;
  try {
    ...
    $q.notify({
      type: 'positive',
      message: res.data?.message || 'Value applied',
      position: 'top',
    });
    await loadReport();
  } catch (e) {
    $q.notify({
      type: 'negative',
      message: e.response?.data?.detail || e.message || 'Failed to apply value',
      position: 'top',
    });
  } finally {
    correcting.value = false;
...
async function markEdited(findingId) {
  const row = (reportGroups.value || [])
    .flatMap((g) => g.findings || [])
    .find((f) => f.id === findingId);
  if (row && !(await confirmReopenIfNeeded(row))) return;
  correcting.value = true;
  try {
    const res = await aiClaimVettingAPI.decideFinding(findingId, {
      decision: 'edited',
      note: 'Reviewed / corrected manually from AI Vetting console',
    });
    $q.notify({
      type: 'positive',
      message: res.data?.message || 'Marked as edited',
      position: 'top',
    });
    await loadReport();
  } catch (e) {
...
async function bulkReopenEdited(findingIds) {
  if (!findingIds?.length) return;
  if (!(await confirmReopenIfNeeded(findingIds))) return;
  const ok = await new Promise((resolve) => {
    $q.dialog({
      title: 'Reopen as AI vetted?',
      message: `Mark ${findingIds.length} finding(s) as edited and reopen any finalized claims as ai_vetted (exportable). Fix Member No / codes on the claim, then export.`,
      cancel: true,
      persistent: true,
    })
      .onOk(() => resolve(true))
  ...
  try {
    ...
    $q.notify({
      type: res.data?.fail_count ? 'warning' : 'positive',
      message: `Reopened/edited: ${res.data?.ok_count || 0} ok${res.data?.fail_count ? `, ${res.data.fail_count} failed` : ''}`,
      position: 'top',
    });
    selectedIds.value = [];
    await loadReport();
    ...
    $q.notify({
      type: 'negative',
      message: e.response?.data?.detail || e.message || 'Bulk reopen failed',
      position: 'top',
    });
  } finally {
    correcting.value = false;
...
async function bulkCorrectSelected() {
  ...
  for (const group of reportGroups.value) {
    for (const f of group.findings || []) {
      ...
      else if (isReviewOnlyFinding(f)) reviewIds.push(f.id);
  ...
  if (skippedChoice) {
    $q.notify({
      type: 'info',
      message: `${skippedChoice} DRG row(s) need a chosen code (use Apply).`,
      position: 'top',
    });
  }
  if (autoIds.length) await bulkCorrect(null, autoIds);
  if (reviewIds.length) await bulkReopenEdited(reviewIds);
}
...
async function draftRuleFromEnglish() {
  const instruction = (rulePlainEnglish.value || '').trim();
  if (instruction.length < 8) {
    $q.notify({ type: 'warning', message: 'Write a short description of the rule first', position: 'top' });
    return;
  }
  draftingRule.value = true;
  ruleDraftNote.value = '';
  try {
    const res = await aiClaimVettingAPI.draftRuleFromText(instruction);
    ...
    ruleForm.value = {
      ...
      enabled: true,
      condition: {
        field: draft.condition?.field || 'memberNo',
        op: draft.condition?.op || 'equals',
        value: draft.condition?.value ?? '',
        skip_if_ghana_card: !!draft.condition?.skip_if_ghana_card,
        skip_if_hin_shaped: !!draft.condition?.skip_if_hin_shaped,
      },
      action_type: draft.action_type || draft.suggested_action?.type || 'review_only',
      finding_template: draft.finding_template || '',
      recommendation_template: draft.recommendation_template || '',
    };
    ruleDraftNote.value =
      res.data?.explanation ||
      'AI drafted a rule below — review fields, then Save rule to activate it.';
    ruleFormOpen.value = true;
    $q.notify({
      type: 'positive',
      message: 'Draft ready — review and Save to approve',
      position: 'top',
    });
  } catch (e) {
    $q.notify({
      type: 'negative',
      message: e.response?.data?.detail || e.message || 'Failed to draft rule',
      position: 'top',
    });
  } finally {
    draftingRule.value = false;
  }
}
