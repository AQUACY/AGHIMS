...
class SurgeryPrice(Base):
    """Surgery price list item model"""
    __tablename__ = "surgery_prices"
    ...
    claim_amount = Column(Float, nullable=True)  # NHIA claim amount from price list
    nhia_claim_co_payment = Column(Float, nullable=True)
    clinic_bill_effective = Column(String(100), nullable=True)
    is_active = Column(Boolean, default=True)
    
    def __repr__(self):
        return f"<SurgeryPrice {self.g_drg_code} - {self.service_name}>"
